...
import type { ApiResponse, WheelData } from "./types";
...
export function pickConnectedBatteryLevel(
  connectType: unknown,
  batteryLevel: unknown,
  previousLevel?: number
): number | undefined {
  if (connectType !== "connected") return previousLevel;
  if (typeof batteryLevel !== "number" || !Number.isFinite(batteryLevel)) return previousLevel;

  const rounded = clamp(Math.round(batteryLevel), 0, 100);
  if (rounded === 0) return previousLevel;
  return rounded;
}
...
function buildCodeMap(raw: unknown): Record<string, unknown> {
...
export async function findWorkingUrl(): Promise<string | null> {
...
export function normalize(raw: ApiResponse): WheelData {
  ...
  const safetyMargin = pickCodeValue(
    source,
    codeMap,
    "vsmg",
    "vsmn",
...
export async function fetchWheelData(
  baseUrl: string,
  sampleIndex = 0
): Promise<{ data: WheelData; ok: boolean; error?: string }> {
