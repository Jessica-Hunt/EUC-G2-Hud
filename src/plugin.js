import { CreateStartUpPageContainer, RebuildPageContainer, waitForEvenAppBridge, } from "@evenrealities/even_hub_sdk";
import { CAPTURE_DOUBLE_CLICK_MS, FAST_TILE_ROLES, HEARTBEAT_MS, INFO_REFRESH_MS, POLL_MS, SIMULATOR_GLASSES_BATTERY, SIMULATOR_URL, TOGGLE_COOLDOWN_MS, } from "./config";
import { blankSlowTextState, buildFastReadoutState, buildSlowTextState, getCaptureEventType, getTime, isSingleToggleEvent, pushFastReadoutState, pushHudTiles, pushSlowTextState, resetHudTileSignatures, summarizeEvenHubEvent, } from "./hud";
...
export async function startPlugin(bridge) {
    ...
    const lastSlowTextSignatures = new Map();
    const kickHudTask = (promise, label) => {
    ...
    const primeHud = (status, state) => {
        kickHudTask(pushHudTiles(bridge, {}, undefined, true), `${status} image push`);
        kickHudTask(pushFastReadoutState(bridge, buildFastReadoutState({}, status, state), lastFastReadoutSignature, true), `${status} fast text push`);
        kickHudTask(pushSlowTextState(bridge, criticalOnlyMode ? blankSlowTextState() : buildSlowTextState({}, latestAuxData), lastSlowTextSignatures, true), `${status} slow text push`);
    };
    const rebuildHudPage = async () => {
        const rebuilt = await bridge.rebuildPageContainer(new RebuildPageContainer(buildHudPageDefinition()));
        if (!rebuilt) {
            return false;
        }
    ...
    if (!workingUrl) {
        window.__setStatus?.("No URL worked. Is EUC World web server on? Check port in Settings.", "error");
    ...
    async function refreshAuxData() {
        try {
            ...
            const glassesBattery = pickConnectedBatteryLevel(device?.status?.connectType, device?.status?.batteryLevel, latestAuxData.glassesBattery);
            latestAuxData = {
                ...latestAuxData,
    ...
    bridge.onDeviceStatusChanged((deviceStatus) => {
        const glassesBattery = pickConnectedBatteryLevel(deviceStatus.connectType, deviceStatus.batteryLevel, latestAuxData.glassesBattery);
        if (glassesBattery === undefined)
            return;
        latestAuxData = {
            ...latestAuxData,
            glassesBattery,
    ...
    const toggleCriticalOnlyMode = () => {
        ...
        window.__setCleanMode?.(criticalOnlyMode);
        window.__setEventDebug?.(JSON.stringify({
        ...
        try {
            while (pendingFastTextRender || pendingTextRender) {
                if (pendingFastTextRender) {
                    ...
                    await pushFastReadoutState(bridge, buildFastReadoutState(latestData, latestStatus, renderState), lastFastReadoutSignature, force);
                    continue;
                }
                if (pendingTextRender) {
                    ...
                    await pushSlowTextState(bridge, criticalOnlyMode
                        ? blankSlowTextState()
                        : buildSlowTextState(latestData, latestAuxData), lastSlowTextSignatures, force);
                }
            }
        }
        finally {
            textQueueRunning = false;
            if (pendingFastTextRender || pendingTextRender)
                void flushTextQueue();
        }
    }
    ...
    async function tick() {
        ...
        try {
            ...
            if (ok) {
                ...
                renderState = { heartbeatOn: false, showHeartbeat: false };
                lastHeartbeatToggleAt = 0;
                window.__setStatus?.(`Fetch failed (${fails}): ${error}`, "error");
