...
import type { FastReadoutState, HudAuxData, RenderState, SlowTextState, WheelData } from "./types";
import { clamp, fmt, fmtInt, getTime, isRecord, normalizeKey, shortStatus } from "./utils";

export function buildSlowTextState(d: WheelData, aux: HudAuxData): SlowTextState {
  ...
  return {
    ...
    summary:
      `TRIP ${fmt(d.trip_distance, 2)}km | ODO ${fmt(d.total_distance, 0)}km | `
...
export function blankSlowTextState(): SlowTextState {
  return {
    trip: HIDDEN_TEXT,
    odo: HIDDEN_TEXT,
...
function osEventTypeLabel(eventType: OsEventTypeList | undefined): string | undefined {
  switch (eventType) {
...
function eventSourceLabel(eventSource: EventSourceType | undefined): string | undefined {
  switch (eventSource) {
...
export function summarizeEvenHubEvent(event: {
  textEvent?: { containerID?: number; containerName?: string; eventType?: OsEventTypeList };
  listEvent?: {
    containerID?: number;
  ...
  sysEvent?: { eventType?: OsEventTypeList; eventSource?: EventSourceType };
...
function readEventSource(payload: Record<string, unknown>): EventSourceType | undefined {
  const numericEventSource = readNumber(payload, "eventSource", "EventSource");
  if (
    numericEventSource === EventSourceType.TOUCH_EVENT_FROM_GLASSES_R
  ...
  if (typeof rawEventSource !== "string") return undefined;
...
function readOsEventType(payload: Record<string, unknown>): OsEventTypeList | undefined {
  const numericEventType = readNumber(payload, "eventType", "Event_Type");
  if (
    numericEventType === OsEventTypeList.CLICK_EVENT
...
export function isSingleToggleEvent(eventType: OsEventTypeList): boolean {
  return (
...
function readCaptureEventType(payload: unknown): OsEventTypeList | undefined {
  ...
  for (const key of ["textEvent", "listEvent", "jsonData", "data", "payload"]) {
    ...
    if (nestedEventType !== undefined) return nestedEventType;
  }

  return undefined;
}

function readSystemToggleEventType(payload: unknown): OsEventTypeList | undefined {
  if (!isRecord(payload)) return undefined;
...
export function getCaptureEventType(event: {
  textEvent?: { containerID?: number; containerName?: string; eventType?: OsEventTypeList };
  listEvent?: { containerID?: number; containerName?: string; eventType?: OsEventTypeList };
  sysEvent?: { eventType?: OsEventTypeList };
  jsonData?: Record<string, unknown>;
}): OsEventTypeList | undefined {
  return (event.listEvent ? readOsEventType(event.listEvent as Record<string, unknown>) ?? OsEventTypeList.CLICK_EVENT : undefined)
    ?? (event.textEvent ? readOsEventType(event.textEvent as Record<string, unknown>) ?? OsEventTypeList.CLICK_EVENT : undefined)
    ?? readCaptureEventType(event.jsonData)
    ?? readSystemToggleEventType(event.sysEvent)
    ?? readSystemToggleEventType(event.jsonData);
}

export function buildFastReadoutState(d: WheelData, status: string, state: RenderState): FastReadoutState {
  ...
  return {
    ...
    dialHeartbeat:
      hasSeparateDialHeartbeat && state.showHeartbeat && state.heartbeatOn
        ? "."
        : "",
  };
}

export async function pushFastReadoutState(
  bridge: EvenAppBridge,
  state: FastReadoutState,
  ...
  if (!force && signatureRef.current === signature) return;
...
export async function pushSlowTextState(
  bridge: EvenAppBridge,
  state: SlowTextState,
  ...
  for (const container of currentSlowTextContainers()) {
    ...
    if (!force && signatures.get(container.id) === content) continue;
...
function decodeBase64Image(dataUrl: string): Uint8Array {
  ...
  if (!base64) throw new Error("Failed to encode HUD image");
...
function initTileContext(): CanvasRenderingContext2D {
...
function drawDynamicTile(d: WheelData): Uint8Array {
  ...
  ctx.beginPath();
  ctx.arc(
    gaugeCenterX + Math.cos(valueAngle) * gaugeRadius,
    gaugeCenterY + Math.sin(valueAngle) * gaugeRadius,
    5,
    0,
    Math.PI * 2
  );
...
function renderTile(role: TileRole, d: WheelData): Uint8Array {
  switch (role) {
    case "dynamic":
...
const lastTileSignatures = new Map<number, string>();

export function resetHudTileSignatures(): void {
  lastTileSignatures.clear();
}

function tileSignature(role: TileRole, d: WheelData): string {
...
async function pushHudTile(
  bridge: EvenAppBridge,
  tile: (typeof IMAGE_TILES)[number],
  d: WheelData,
  force = false
): Promise<void> {
  const signature = tileSignature(tile.role, d);
  if (!force && lastTileSignatures.get(tile.id) === signature) return;
...
export async function pushHudTiles(
  ...
  const allowedRoles = roles ? new Set<TileRole>(roles) : null;
  for (const tile of IMAGE_TILES) {
    if (allowedRoles && !allowedRoles.has(tile.role)) continue;
